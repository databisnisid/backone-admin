...
class StarlinkAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('MSISDN', {
            'classes': ('collapse',),
            'fields': ('service_line_number', 'kit_serialnumber',
                       ('quota_usage', 'quota_day', 'quota_date', 'updated_at')
                       )
        }),
        ('Site dan Keterangan', {
            'classes': ('collapse',),
    ...
    list_display = ['service_line_number', 'kit_serialnumber', 'site', 'quota_usage', 'quota_day', 'updated_at', 'additional_info']
