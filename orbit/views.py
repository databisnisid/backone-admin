...
from dsc.models import DscDpi, DscDpiProit
...
def get_quota(request, msisdn):
    ...
    if not qs:
        qs = DscDpiProit.objects.filter(msisdn__contains=msisdn).values(
            "id",
            "msisdn",
            "quota_total",
            "quota_current",
